...
extensions = ["cogs.gearcalcs", "cogs.randomizers"]
...
async def weaponglossary(ctx):
    ...
    for x in range(
        24
    ):  # uses 2 for loops since the glossary must be sent in 2 messages to be under 2000 characters
...
async def creators(ctx):
    await ctx.send(
        "Annie was created by Chaedr and Quark. They can be found on Twitter at @chaedr_ and @StrangeQuarked, respectively."
    )
...
async def selfcare(ctx):
    await ctx.send(
        "Please make sure you have hydrated, eaten sufficient food, moved around if you have been sitting for a while, slept if you are tired, taken any necessary medication and done any other actions needed to take care of yourself. Self-care is very important. I might be a robot but I still care about you :)."
    )
