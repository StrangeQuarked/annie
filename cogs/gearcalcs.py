...
class Gearcalcs(commands.Cog):
    ...
    def calcPctChange(self, basevalue, newvalue):#to use: input the starting value without any ability chunks, then the value with ability chunks 
      return (-100 * (1-(newvalue/basevalue)))
    
    def InterpolationFunction(self, b, c):#b and c are p/100 and s as calculated in the calcAbilityEffects function
      if c == 0.5:
        return(b)
      elif b == 0.0:
        return(b)
        ...
        return(2.71828182846  ** (-(math.log(b) * math.log(c))/math.log(2)))#log is the same as what is normally ln (natural logarithm or log base e) here
    ...
    @commands.command()
    async def gearcalc(self, ctx, ability=None, AP="", AdditionalInput="", AdditionalInput2=""):
      if ability == None:
        await ctx.send("Error: No ability entered.")
      elif ability == "help":
        if AP in self.abilitydict and AP != "":
          if AP == "mpu":
            await ctx.send("Calculates the effects of Main Power Up on a certain weapon. Usage: `a.gearcalc mpu (amount) (weapon)`. Use `a.weaponglossary` for a list of valid weapon names.")
          elif AP == "spu":
            await ctx.send("Calculates the effects of Special Power Up on a certain special. Usage: `a.gearcalc spu (amount) (special)`. Use `a.subspecialglossary` for a list of valid special weapon names.")
          elif AP == "bpu":
            await ctx.send("Calculates the effects of sub power up on a given sub weapon. Usage: `a.gearcalc bpu (amount) (sub)`. Use `a.subspecialglossary` for a list of valid sub weapon names.")
          elif AP == "res":
            ...
            await ctx.send("Calculates the effects of Run Speed Up given a weapon. Can be modified by Comeback and Opening Gambit. Usage: `a.gearcalc rsu (amount) (weapon) [cbk/og]`.")
          elif AP == "ssu":
            await ctx.send("Calculates the effect of Swim Speed Up given a weight class. Can be modified Comeback, Opening Gambit and Ninja Squid. Weight classes are 'lightweight', 'middleweight', 'heavyweight', and 'rainmaker'. Usage: `a.gearcalc ssu (amount) (weight class) [cbk/og/ns]`.")
          elif AP == "qsj":
            await ctx.send("Calculates the effects of Quick Super Jump. Usage: `a.gearcalc qsj (amount)`.")
          elif AP == "qr":
            await ctx.send("Calculates the effects of Quick Respawn. Usage: 'a.gearcalc qr (amount)`.'")
          else:
            await ctx.send("Invalid ability entered. Use 'a.abilityglossary' for accepted ability names.")
        else:
    ...
    def calcbdef(self, ctx, AP):
      ...
      for bomb in bombs.keys():
        if "Burst Bomb" in bomb:
          ...
          if bombs[bomb] * (1-amnt/60) < 100 and (bomb == "Splat Bomb (near)" or bomb == "Suction Bomb (near)" or bomb == "Autobomb (near)" or bomb == "Curling Bomb (near)"):
            results[bomb] = 100
          else:
      ...
      for special in specials.keys():
        damage = round(self.calcAbilityEffects(AP, specials[special][0], specials[special][1]),2)
        if damage < 100 and (special == "Tenta Missiles (hit)" or special == "Baller (hit)" or special == "Splashdown (hit)" or special == "Inkjet (hit)"):
          damage = 100
    ...
    def calcinkres(self, ctx, AP, OpeningGambit=""): #calculates the effects of ink resistance given an amount of ability points  
        ...
        pctchange4= round(self.calcPctChange(0.8, JumpHeight),2)#no %change for invulnerability time because of a division by 0 error
    ...
    def calcqsj(self, ctx, AP):
      ...
      if AP > 57:
        AP = 57
      kid = round(self.calcAbilityEffects(AP, 3.98, 2.29),2)#separate variables for readability
    ...
    def calcspu(self, ctx, AP, specialtype): 
      ...
      if specialtype not in specials:
        ...
        if specialtype == "baller":
          ...
          ActivationTime = self.calcAbilityEffects(AP, 1.5, 0.5)#units are seconds
          ...
          InkCoverageIncrease = self.calcAbilityEffects(AP, 100, 120)-100#units are a percentage
    ...
    def calcssu(self, ctx, AP, weightclass, modifier=""):#calculates swim speed given an amount of ability chunks, the weapon's weight class and another modifier like ninja squid if applicable
    ...
    def calcrsu(self, ctx, AP, weapon, modifier=""):#calculates the user's run speed given an amount of ability chunk and weapon weight class
      if modifier == "opening-gambit" or modifier == "og":
        AP = self.calcAP(AP) + 30
      elif modifier == "comeback" or modifier == "cbk":
        AP = self.calcAP(AP) + 10
      ...
      NoStrafingSpeedWeapons = ["roller", "carbon", "dynamo", "flingza","rainmaker", "inkbrush", "octobrush"]
      if weapon == "hydra" or weapon == "dynamo" or weapon == "eliter" or weapon == "eliter-scope" or weapon == "explosher" or weapon == "tent" or weapon == "hydra":
        weightclass = "heavyweight"
      elif weapon == "aerospray" or weapon == "bamboozler" or weapon == "carbon" or weapon == "clash" or weapon == "jr" or weapon == "inkbrush" or weapon == "luna" or weapon == "nzap" or weapon == "splash" or weapon == "sploosh" or weapon == "tri" or weapon == "undercover":
      ...
      if weapon in NoStrafingSpeedWeapons:
        ...
        StrafeSpeed = self.calcAbilityEffects(AP, StrafingSpeeds[weapon][0], StrafingSpeeds[weapon][1])
        pctchange2 = round(self.calcPctChange(StrafingSpeeds[weapon][0], StrafeSpeed),2)
    ...
    def calcbpu(self, ctx, AP, SubType):
      ...
      if AP > 57:
        AP = 57
      SubWeapons = ["splat-bomb", "burst-bomb", "suction-bomb", "autobomb", "curling-bomb","mine", "beakon", "fizzy-bomb", "toxic-mist", "wall", "sprinkler", "torpedo", "sensor"]
      if SubType in SubWeapons:
        if SubType == "splat-bomb" or SubType == "burst-bomb" or SubType == "autobomb" or SubType == "curling-bomb" or SubType == "toxic-mist" or SubType == "suction-bomb":
    ...
    def calcism(self, ctx, AP, weapon, modifier=""):
      ...
      if weapon == "flingza" or weapon == "squeezer" or weapon == "l-3" or weapon == "h-3":
    ...
    def calcmpu(self, ctx, AP, Weapon):
      ...
      HeavyNautMini = {
        "heavy": [1.2, 1.49, 2.4, 2.98],#units are seconds, with the minimum and maximum first and second ring burst duration
        "nautilus": [0.87, 1.13, 1.73, 2.25],
        "mini": [0.6, 0.81, 1.2, 1.62]
      }
      Brushes = {
        "inkbrush": [1.92, 2.016, 100, 148],#units are dash speed in DU/frame and % trail ink coverage from base value
        "octobrush": [1.68, 1.8816, 100, 180]
      }
      SquifferEliter = {
        "eliter": [290.5, 305.5, 100, 117],#units are max range and percent ink coverage from base value
        "eliter-scope": [310.5, 325.5, 100, 117],
        "squiffer": [167.65, 182.65, 100, 124]
      }
      ChargerGooBamboo = {
        "charger": [40, 48, 80, 96],#units are the damage at min partial charge and max partial charge
        "scope":  [40, 48, 80, 96],
        "gootuber": [40, 46, 130, 149.5],
        "bamboozler": [30, 36, 85, 99.9]
      }
      Shot52 = {
        "splattershot": [100, 50, 100, 80],#units are percentage of base value of jump and ground shot randomization
        "52":  [100, 50, 100, 80]
      }
      BlasterRangeClash = {
        "blaster": [100, 60],#units are percentage of base value of jump shot randomization
        "range-blaster": [100, 62.5],
        "clash": [100, 50]
      }
      Rollers = {
        "roller": [100, 115],#units are % of base value of damage for now since mpu effects for roller seem to differ between gear calculators
        "carbon": [100, 115],
        "flingza": [100,115],
        "dynamo": [100, 115]
      }
      ...
      if Weapon in list(DamageBuffWeapons.keys()):
        Damage = self.calcAbilityEffects(AP, DamageBuffWeapons[Weapon][0], DamageBuffWeapons[Weapon][1])
        pctchange = round(self.calcPctChange(DamageBuffWeapons[Weapon][0], Damage),2)
        ...
        RingBurstTime1 = self.calcAbilityEffects(AP, HeavyNautMini[Weapon][0], HeavyNautMini[Weapon][1])
        pctchange1 = round(self.calcPctChange(HeavyNautMini[Weapon][0], RingBurstTime1),2)
        RingBurstTime2 = self.calcAbilityEffects(AP, HeavyNautMini[Weapon][2], HeavyNautMini[Weapon][3])
        pctchange2 = round(self.calcPctChange(HeavyNautMini[Weapon][2], RingBurstTime2),2)
        ...
        DashSpeed = self.calcAbilityEffects(AP, Brushes[Weapon][0], Brushes[Weapon][1])
        pctchange1 = round(self.calcPctChange(Brushes[Weapon][0], DashSpeed),2)
        InkTrailSize = round(self.calcAbilityEffects(AP, Brushes[Weapon][2], Brushes[Weapon][3]),2)
        ...
        MaxRange = self.calcAbilityEffects(AP, SquifferEliter[Weapon][0], SquifferEliter[Weapon][1])
        pctchange = round(self.calcPctChange(SquifferEliter[Weapon][0], MaxRange),2)
        Painting = round(self.calcAbilityEffects(AP, SquifferEliter[Weapon][2], SquifferEliter[Weapon][3]),2)
        ...
        MinChargeDamage = self.calcAbilityEffects(AP, ChargerGooBamboo[Weapon][0], ChargerGooBamboo[Weapon][1])
        pctchange1 = round(self.calcPctChange(ChargerGooBamboo[Weapon][0], MinChargeDamage),2)
        MaxPartialChargeDamage = self.calcAbilityEffects(AP, ChargerGooBamboo[Weapon][2], ChargerGooBamboo[Weapon][3])
        pctchange2 = round(self.calcPctChange(ChargerGooBamboo[Weapon][2], MaxPartialChargeDamage))
        ...
        JumpRNG = round(self.calcAbilityEffects(AP, Shot52[Weapon][0], Shot52[Weapon][1]),2)
        GroundRNG = round(self.calcAbilityEffects(AP, Shot52[Weapon][2], Shot52[Weapon][3]),2)
        ...
        JumpRNG = round(self.calcAbilityEffects(AP, BlasterRangeClash[Weapon][0], BlasterRangeClash[Weapon][1]),2)
        return(f"Jump shot randomization {JumpRNG}% of base value.")
      elif Weapon in list(Rollers.keys()):
        Damage = round(self.calcAbilityEffects(AP, Rollers[Weapon][0], Rollers[Weapon][1]),2)
        return(f"Roller damage: {Damage}% of base damage.")
      elif Weapon == "tent":
        ShieldHealth = self.calcAbilityEffects(AP, 700, 1000)#units are tent shield hp
        ...
        BlastRadius = round(self.calcAbilityEffects(AP, 100, 300),2)#units are percentage of base value
        ...
        BlastRadius = round(self.calcAbilityEffects(AP, 100, 106.061),2)#units are percentage of base value
        ...
        GroundRNG = round(self.calcAbilityEffects(AP, 100, 80),2)#units are percentage of base value
